  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        cond, "body", body, "output_shapes", output_shapes,
...
def StatelessWhile(input, cond, body, output_shapes=[], parallel_iterations=10, name=None):
  return stateless_while(input=input, cond=cond, body=body, output_shapes=output_shapes, parallel_iterations=parallel_iterations, name=name)
...
def stateless_while_eager_fallback(input, cond, body, output_shapes=[], parallel_iterations=10, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
