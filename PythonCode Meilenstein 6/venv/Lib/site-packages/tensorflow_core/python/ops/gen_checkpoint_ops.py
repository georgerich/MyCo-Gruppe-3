...
def GenerateVocabRemapping(new_vocab_file, old_vocab_file, new_vocab_offset, num_new_vocab, old_vocab_size=-1, name=None):
  return generate_vocab_remapping(new_vocab_file=new_vocab_file, old_vocab_file=old_vocab_file, new_vocab_offset=new_vocab_offset, num_new_vocab=num_new_vocab, old_vocab_size=old_vocab_size, name=name)
GenerateVocabRemapping.__doc__ = generate_vocab_remapping.__doc__
GenerateVocabRemapping = _doc_controls.do_not_generate_docs(_kwarg_only(GenerateVocabRemapping))
...
def LoadAndRemapMatrix(ckpt_path, old_tensor_name, row_remapping, col_remapping, initializing_values, num_rows, num_cols, max_rows_in_memory=-1, name=None):
  return load_and_remap_matrix(ckpt_path=ckpt_path, old_tensor_name=old_tensor_name, row_remapping=row_remapping, col_remapping=col_remapping, initializing_values=initializing_values, num_rows=num_rows, num_cols=num_cols, max_rows_in_memory=max_rows_in_memory, name=name)
LoadAndRemapMatrix.__doc__ = load_and_remap_matrix.__doc__
LoadAndRemapMatrix = _doc_controls.do_not_generate_docs(_kwarg_only(LoadAndRemapMatrix))
