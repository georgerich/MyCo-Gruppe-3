  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        name, _ctx._post_execution_callbacks, in_tensors, "num_batch_threads",
...
def BatchFunction(in_tensors, captured_tensors, f, num_batch_threads, max_batch_size, batch_timeout_micros, Tout, max_enqueued_batches=10, allowed_batch_sizes=[], container="", shared_name="", batching_queue="", name=None):
  return batch_function(in_tensors=in_tensors, captured_tensors=captured_tensors, f=f, num_batch_threads=num_batch_threads, max_batch_size=max_batch_size, batch_timeout_micros=batch_timeout_micros, Tout=Tout, max_enqueued_batches=max_enqueued_batches, allowed_batch_sizes=allowed_batch_sizes, container=container, shared_name=shared_name, batching_queue=batching_queue, name=name)
BatchFunction.__doc__ = batch_function.__doc__
BatchFunction = _doc_controls.do_not_generate_docs(_kwarg_only(BatchFunction))
tf_export("raw_ops.BatchFunction")(BatchFunction)
...
def Unbatch(batched_tensor, batch_index, id, timeout_micros, container="", shared_name="", name=None):
  return unbatch(batched_tensor=batched_tensor, batch_index=batch_index, id=id, timeout_micros=timeout_micros, container=container, shared_name=shared_name, name=name)
Unbatch.__doc__ = unbatch.__doc__
Unbatch = _doc_controls.do_not_generate_docs(_kwarg_only(Unbatch))
tf_export("raw_ops.Unbatch")(Unbatch)
...
def unbatch_grad(original_input, batch_index, grad, id, container="", shared_name="", name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "UnbatchGrad", name, _ctx._post_execution_callbacks, original_input,
...
def UnbatchGrad(original_input, batch_index, grad, id, container="", shared_name="", name=None):
  return unbatch_grad(original_input=original_input, batch_index=batch_index, grad=grad, id=id, container=container, shared_name=shared_name, name=name)
UnbatchGrad.__doc__ = unbatch_grad.__doc__
UnbatchGrad = _doc_controls.do_not_generate_docs(_kwarg_only(UnbatchGrad))
tf_export("raw_ops.UnbatchGrad")(UnbatchGrad)
